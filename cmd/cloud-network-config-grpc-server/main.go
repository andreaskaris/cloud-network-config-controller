...
import (
	...
	daemonsetcontroller "github.com/openshift/cloud-network-config-controller/pkg/controller/daemonset"
	...
	leaderelection.RunOrDie(ctx, leaderelection.LeaderElectionConfig{
		...
		Callbacks: leaderelection.LeaderCallbacks{
			OnStartedLeading: func(ctx context.Context) {
				...
				/*
					var cnccImage string
					deployment, err := kubeClient.AppsV1().Deployments(controllerNamespace).Get(context.TODO(), controllerName, v1.GetOptions{})
					if err != nil {
						klog.Exitf("Error getting the CNCC deployment: %v", err)
					}
				...
				daemonsetController := daemonsetcontroller.NewDaemonSetController(
					ctx,
					kubeClient,
				...
				go func() {
					defer wg.Done()
					if err = daemonsetController.Run(stopCh); err != nil {
						klog.Exitf("Error running DaemonSet controller: %s", err.Error())
					}
				}()
				wg.Add(1)
